...
- **Reroute requires Ollama.** When budget pressure triggers reroute and no local Ollama model is available, the gate falls back to `halt`. Check the `local_model` field in the `l6e_session_start` response — if it is `null`, reroutes will halt instead.
